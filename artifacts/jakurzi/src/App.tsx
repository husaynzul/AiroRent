...
function WishlistPage() {
  ...
  return <main className="min-h-[calc(100dvh-68px)] bg-[#fffafb] px-5 pb-28 pt-8 lg:mx-auto lg:min-h-0 lg:max-w-[720px] lg:bg-transparent lg:px-8 lg:py-10">
...
function RecentBookingCard({ booking }: { booking: BookingRecord }) {
  ...
  return <article className="flex gap-3 rounded-[22px] border border-black/[.05] bg-white p-3 shadow-[0_6px_22px_rgba(0,0,0,.06)]" data-testid={`card-booking-${booking.id}`}>
    ...
    <button onClick={open} className="min-w-0 flex-1 text-left" data-testid={`button-booking-open-${booking.id}`}>
      <span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold ${booking.status === 'Completed' ? 'bg-[#e4f6e8] text-[#23813b]' : booking.status === 'Cancelled' ? 'bg-[#f3f3f3] text-black/50' : 'bg-[hsl(var(--primary)/.1)] text-[hsl(var(--primary))]'}`}>{booking.status}</span>
      <h2 className="mt-2 line-clamp-2 text-[16px] font-bold leading-tight">{listing.title}</h2>
      <p className="mt-1 flex items-center gap-1 truncate text-[11px] text-black/55"><MapPin size={12} />{listing.location.split(',')[0]}</p>
...
function ProfileExperiencePage() {
  ...
  const userName = readUserName();
  const notify = () => setToast('You are all caught up.');
  const row = (label: string, Icon: typeof Settings, action: () => void, testId: string) => <button onClick={action} key={label} className="flex w-full items-center gap-4 border-b border-black/[.08] py-[16px] text-left last:border-b-0" data-testid={testId}><span className="grid size-10 place-items-center rounded-full bg-[hsl(var(--secondary))] text-[hsl(var(--primary))]"><Icon size={20} strokeWidth={1.8} /></span><span className="flex-1 text-[16px] font-medium">{label}</span><ChevronRight size={20} className="text-black/45" /></button>;
  const profileRows = [
  ...
  return <main className="min-h-[calc(100dvh-68px)] bg-[#fff8fb] pb-28 md:mx-auto md:min-h-0 md:max-w-[940px] md:bg-transparent md:px-8 md:py-10">
    ...
    <section className="relative -mt-12 mx-5 rounded-[25px] bg-white p-5 shadow-[0_7px_24px_rgba(0,0,0,.08)] md:mx-0" data-testid="section-profile-trips-overview">
      ...
      <div className="mt-5 grid grid-cols-3 divide-x divide-black/[.08]"><button onClick={() => setLocation('/trips')} className="text-center" data-testid="button-profile-overview-upcoming"><span className="mx-auto grid size-12 place-items-center rounded-2xl bg-[hsl(var(--secondary))] text-[hsl(var(--primary))]"><BriefcaseBusiness size={22} /></span><b className="mt-2 block text-xl">4</b><span className="text-xs text-black/55">Upcoming</span></button><button onClick={() => setLocation('/trips')} className="text-center" data-testid="button-profile-overview-completed"><span className="mx-auto grid size-12 place-items-center rounded-2xl bg-[hsl(var(--secondary))] text-[hsl(var(--primary))]"><CalendarDays size={22} /></span><b className="mt-2 block text-xl">7</b><span className="text-xs text-black/55">Completed</span></button><button onClick={() => setLocation('/wishlist')} className="text-center" data-testid="button-profile-overview-wishlisted"><span className="mx-auto grid size-12 place-items-center rounded-2xl bg-[hsl(var(--secondary))] text-[hsl(var(--primary))]"><Heart size={22} /></span><b className="mt-2 block text-xl">{readSaved().length}</b><span className="text-xs text-black/55">Wishlisted</span></button></div>
    </section>
    <section className="mx-5 mt-5 rounded-[23px] bg-white px-4 shadow-[0_6px_20px_rgba(0,0,0,.06)] md:mx-0">{profileRows.map(([label, Icon, action, testId]) => row(label, Icon, action, testId))}</section>
    <button onClick={() => setToast('Log out is ready to confirm.')} className="mx-5 mt-5 flex w-[calc(100%-2.5rem)] items-center gap-4 rounded-[23px] bg-white px-4 py-4 text-left shadow-[0_6px_20px_rgba(0,0,0,.06)] md:mx-0 md:w-full" data-testid="button-profile-overview-logout"><span className="grid size-10 place-items-center rounded-full bg-[hsl(var(--secondary))] text-[hsl(var(--primary))]"><DoorOpen size={20} /></span><span className="flex-1 text-[16px] font-medium">Log Out</span><ChevronRight size={20} className="text-black/45" /></button>
    {toast && <Toast text={toast} onClose={() => setToast('')} />}
  </main>;
}
