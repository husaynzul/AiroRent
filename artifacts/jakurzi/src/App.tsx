  ...
  return <main className="mx-auto max-w-[1260px] px-5 pb-28 pt-5 lg:px-8">
    <div className="md:hidden">
      ...
      <section className="relative -mx-5 -mt-7 rounded-t-[30px] bg-white px-6 pb-28 pt-7">
        ...
        <div className="fixed inset-x-0 bottom-0 z-30 flex items-center justify-between gap-4 border-t border-black/[.08] bg-white/95 px-5 pb-[max(14px,env(safe-area-inset-bottom))] pt-3 backdrop-blur-xl">
          <div><p className="text-[22px] font-bold">{listing.price}<span className="ml-1 text-sm font-normal text-black/55">{listing.mode === 'Short Let' ? '/ night' : ''}</span></p><p className="text-xs text-black/55">Total before taxes</p></div>
          <button onClick={openBooking} className="rounded-2xl bg-[hsl(var(--primary))] px-5 py-4 text-[16px] font-bold text-white" data-testid="button-detail-check-availability-mobile">{listing.mode === 'Buy' ? 'Make an offer' : 'Check availability'}</button>
        </div>
      </section>
    </div>
    <div className="hidden md:block">
