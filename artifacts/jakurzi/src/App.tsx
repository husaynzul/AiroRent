...
import whatsappIconSrc from '@assets/whatsapp-original-cropped.png';
...
function PhotoViewer({ listing, gallery, selectedImageIndex, onClose, onPrevious, onNext, onSelect }: { listing: Listing; gallery: string[]; selectedImageIndex: number; onClose: () => void; onPrevious: () => void; onNext: () => void; onSelect: (index: number) => void }) {
  const [touchStartX, setTouchStartX] = useState<number | null>(null);
  const handleTouchEnd = (clientX: number) => {
    if (touchStartX === null) return;
    const delta = touchStartX - clientX;
  ...
  return <div className="fixed inset-0 z-[60] bg-[hsl(var(--foreground)/.96)] p-4 text-white backdrop-blur-md" onClick={onClose} data-testid="photo-viewer">
    <div className="mx-auto flex h-full max-w-6xl flex-col" onClick={(event) => event.stopPropagation()}>
      <div className="flex items-center justify-between">
        <div><p className="text-xs font-bold uppercase tracking-[.16em] text-white/60">Photo tour</p><p className="mt-1 text-sm font-semibold">{selectedImageIndex + 1} of {gallery.length}</p></div>
        <button onClick={onClose} className="grid size-11 place-items-center rounded-full bg-white/10 transition hover:bg-white/20" aria-label="Close photo tour" data-testid="button-photo-viewer-close"><X size={22} /></button>
      </div>
      <div className="relative flex min-h-0 flex-1 items-center justify-center py-5" onTouchStart={(event) => setTouchStartX(event.touches[0].clientX)} onTouchEnd={(event) => handleTouchEnd(event.changedTouches[0].clientX)}>
        <button onClick={onPrevious} className="absolute left-0 z-10 grid size-11 place-items-center rounded-full bg-white/10 transition hover:bg-white/20 sm:left-2" aria-label="Previous photo" data-testid="button-photo-viewer-previous"><ChevronLeft size={24} /></button>
        <img src={gallery[selectedImageIndex]} alt={`${listing.title} full-screen view ${selectedImageIndex + 1}`} className="max-h-[72dvh] max-w-[calc(100%-5rem)] rounded-2xl object-contain shadow-2xl sm:max-h-[76dvh]" />
        <button onClick={onNext} className="absolute right-0 z-10 grid size-11 place-items-center rounded-full bg-white/10 transition hover:bg-white/20 sm:right-2" aria-label="Next photo" data-testid="button-photo-viewer-next"><ChevronRight size={24} /></button>
      </div>
      <div className="grid grid-cols-3 gap-2 pb-2 sm:mx-auto sm:max-w-md">{gallery.map((image, index) => <button key={image + index} onClick={() => onSelect(index)} className={`aspect-[1.25] overflow-hidden rounded-xl border-2 transition ${index === selectedImageIndex ? 'border-white' : 'border-transparent opacity-60 hover:opacity-100'}`} aria-label={`Show full photo ${index + 1}`} data-testid={`button-photo-viewer-thumbnail-${index}`}><img src={image} alt="" className="size-full object-cover" /></button>)}</div>
    </div>
  </div>;
}
