    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-[hsl(var(--border))] bg-[hsl(var(--card)/.96)] px-3 pb-[max(9px,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl md:hidden">
      <div className="mx-auto flex max-w-md justify-between">
        {items.map(({ href, label, icon: Icon }) => {
          ...
          return <Link key={href} href={href} className={`flex min-w-[54px] flex-col items-center gap-1 text-[10px] font-semibold transition ${active ? 'text-[hsl(var(--brand-accent))]' : 'text-[hsl(var(--muted-foreground))]'}`} data-testid={`link-bottom-${label.toLowerCase()}`}>
            <span className={`relative grid size-8 place-items-center rounded-full ${active ? 'bg-[hsl(var(--brand-accent)/.1)]' : ''}`}><Icon size={20} strokeWidth={active ? 2.5 : 1.8} />{href === '/messages' && <span className="absolute -right-1 -top-1 grid size-4 place-items-center rounded-full bg-[hsl(var(--brand-accent))] text-[9px] font-bold text-white">3</span>}</span>{label}
          </Link>;
        })}
      </div>
    </nav>
...
function MobileSearchExperience({ mode, setMode, query, setQuery, onClose, onSearch, onOpenFilters }: { mode: Mode; setMode: (mode: Mode) => void; query: string; setQuery: (query: string) => void; onClose: () => void; onSearch: (nextQuery?: string) => void; onOpenFilters: () => void }) {
  ...
  return <div className="fixed inset-0 z-30 h-[100dvh] overflow-y-auto bg-[#f7f8f9] pb-[150px] text-[#1d2329] md:hidden" data-testid="mobile-search-experience">
    <div className="mx-auto max-w-md px-7 pb-8 pt-5">
      ...
      <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
        {(['Rent', 'Buy', 'Short Let'] as Mode[]).map((item) => <button key={item} onClick={() => setMode(item)} className={`shrink-0 rounded-full px-4 py-2 text-[12px] font-bold ${mode === item ? 'bg-[#252b31] text-white' : 'bg-white text-black/55'}`} data-testid={`button-search-experience-mode-${item.toLowerCase().replace(' ', '-')}`}>{item}</button>)}
      </div>
      ...
      <section className="mt-7">
        <div className="flex items-center justify-between"><h2 className="text-[14px] font-bold">Popular destinations</h2><button onClick={() => setActiveCategory('All')} className="flex items-center gap-1 text-[11px] font-semibold text-black/60" data-testid="button-search-view-all">View all <ChevronRight size={14} /></button></div>
